import { HttpException, HttpStatus, Injectable, Post, Query } from '@nestjs/common';
...
export class ArticleService {
  ...
  async updateArticle(slug: string, updateArticleDto: CreateArticleDto, currentUserId: number): Promise<ArticleEntity>{
    ...
    if(!article){
      throw new HttpException('Artcle does not exist', HttpStatus.NOT_FOUND)
    }
  ...
  async addArticleToFavourites(slug: string, currentUserId: number): Promise<ArticleEntity> {
    ...
    const isNotFavourited = user.favorites.findIndex(articleInFavorites => articleInFavorites.id === article.id) === -1
